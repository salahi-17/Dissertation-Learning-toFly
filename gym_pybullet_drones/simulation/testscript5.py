...
def run(
    ...
    INIT_XYZS = np.array([[0, 0, 0],[-1, -0.5, .5]])
    # INIT_XYZS = np.array([[0, 0, 0]])
    target_position = np.array([0, 1, 0])
    ...
    PERIOD = 1  # Period to travel back and forth
    NUM_WP = control_freq_hz * PERIOD
    WAYPOINTS = np.array([INIT_XYZS[0], target_position])
    wp_counters = np.array([0, 0])
    TARGET_POS = np.zeros((NUM_WP,3))
    
    for i in range(NUM_WP):
        TARGET_POS[i, :] = [0.5*np.cos(2*np.pi*(i/NUM_WP)), 0.5*np.cos(2*np.pi*(i/NUM_WP)), 0]
    ...
    for i in range(0, 1000):
        ...
        for j in range(2):
            target_pos = env.get_target_position(j)
